#!/usr/bin/env python3
import os, logging, arcpy
from arcpy.sa import Abs, Con, Float, Raster, SetNull, IsNull, Int  # noqa
import disturbance_config as cfg

SAVE_PCT_AS_INT16 = False  # set True to store in 0.1% units
PCT_SCALE = 10             # -100..100% -> -1000..1000

def _exists(p): return bool(p) and (arcpy.Exists(p) or os.path.exists(p))
def _tcc(y): p = cfg.NLCD_TCC_RASTERS.get(y); return p if _exists(p) else None
def _set_env(ds):
    d = arcpy.Describe(ds); arcpy.env.snapRaster = ds; arcpy.env.cellSize = ds
...
def _pct_breaks(): return getattr(cfg, "NLCD_TCC_PCT_SEVERITY_BREAKS", cfg.NLCD_TCC_SEVERITY_BREAKS)
def _classify_loss(loss_r):
    b1,b2,b3,b4 = sorted(_pct_breaks())
    return Con(loss_r <= 0, 0,
           Con(loss_r <= b1, 1,
           Con(loss_r <= b2, 2,
           Con(loss_r <= b3, 3, Con(loss_r <= b4, 4, 4)))))

def main():
    logging.info("Starting NLCD TCC percent-change processing.")
    arcpy.env.overwriteOutput = True; arcpy.env.pyramid = "NONE"; arcpy.CheckOutExtension("Spatial")
    try:
        if not _exists(cfg.NLCD_RASTER): raise FileNotFoundError(cfg.NLCD_RASTER)
        ...
        for pname, years in cfg.TIME_PERIODS.items():
            ...
            sp, ep = _tcc(s), _tcc(e)
            if not (sp and ep):
                logging.error("Skipping %s (missing TCC).", pname); continue
            ...
            pct = SetNull(IsNull(s_ok) | IsNull(e_ok),
                          Float((e_ok - s_ok) / s_ok * 100))
