...
for region in regions:
    ...
    if layer_name:
        ...
        for period_name, years in time_periods.items():
            ...
            # Build SQL query to filter SURVEY_YEAR and calculate damage_val
            years_str = ','.join(map(str, years))
            sql_query = f"SELECT *, CASE WHEN DAMAGE_TYPE = 'Mortality - Previously Undocumented' THEN 2 ELSE 1 END AS damage_val FROM '{layer_name}' WHERE SURVEY_YEAR IN ({years_str})"
            ...
            # Rasterize the data to create the output raster for the region and time period
            cmd = [
                'gdal_rasterize',
                '-a', 'damage_val',
                '-tr', str(nlcd_res[0]), str(nlcd_res[1]),
                '-te', str(nlcd_bounds.left), str(nlcd_bounds.bottom), str(nlcd_bounds.right), str(nlcd_bounds.top),
                '-ot', 'Byte',
                '-co', 'COMPRESS=LZW',
                '-co', 'TILED=YES',
...
# After processing all regions, merge rasters for each time period using GDAL
